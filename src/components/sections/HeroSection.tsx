...
const HeroSection = () => {
  ...
  const openWhatsApp = (type: 'trial' | 'activate') => {
    ...
    const message = type === 'trial' ? 'Olá! Gostaria de solicitar o teste grátis de 4 horas do Quantum Play.' : 'Olá! Quero ativar meu plano do Quantum Play por R$ 29,90/mês.';
  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-particles">
      ...
      <div className="absolute inset-0">
        <img src={heroImage} alt="Quantum Play IPTV Streaming" className="w-full h-full object-cover opacity-20" />
      ...
      <div className="relative z-10 container mx-auto px-6 text-center">
        <div className={`transition-all duration-1000 ${isVisible ? 'animate-slide-up' : ''}`}>
          ...
          <p className={`text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto mb-12 leading-relaxed animate-fade-in stagger-2`}>
            Mais de <span className="text-primary font-bold">5.700 canais</span> e milhares de filmes e séries em{' '}
          ...
          <div className={`flex justify-center items-center gap-8 mt-12 text-sm text-muted-foreground animate-fade-in stagger-4`}>
            ...
            <div className="flex items-center gap-2">
              <Infinity className="w-4 h-4 text-primary animate-bounce" style={{
              animationDelay: '0.1s'
            }} aria-hidden="true" />
              <span>Sem Fidelidade</span>
            </div>
            <div className="flex items-center gap-2">
              <Monitor className="w-4 h-4 text-primary animate-bounce" style={{
              animationDelay: '0.2s'
            }} aria-hidden="true" />
